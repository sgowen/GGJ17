...
Direct3DGeometryGpuProgramWrapper::Direct3DGeometryGpuProgramWrapper() : GpuProgramWrapper()
{
	...
	auto blob = DX::ReadData(L"ColorVertexShader.cso");
	...
	blob = DX::ReadData(L"ColorPixelShader.cso");
...
void Direct3DGeometryGpuProgramWrapper::bind()
{
	...
	memcpy(mappedResource.pData, &D3DManager->getColorVertices().front(), sizeof(COLOR_VERTEX) * D3DManager->getColorVertices().size());
