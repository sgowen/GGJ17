...
public:
    ...
    virtual void SetWindow(CoreWindow^ window)
    {
        ...
        m_game->Initialize(reinterpret_cast<IUnknown*>(window), outputWidth, outputHeight, m_DPI, rotation);
...
protected:
	...
	{
		INPUT_MANAGER->onTouch(DOWN, e->CurrentPoint->Position.X, e->CurrentPoint->Position.Y);
	}

	void OnPointerMoved(CoreWindow^ sender, PointerEventArgs^ e)
	{
		INPUT_MANAGER->onTouch(DRAGGED, e->CurrentPoint->Position.X, e->CurrentPoint->Position.Y);
	}

	void OnPointerReleased(CoreWindow^ sender, PointerEventArgs^ e)
	{
		INPUT_MANAGER->onTouch(UP, e->CurrentPoint->Position.X, e->CurrentPoint->Position.Y);
	}
...
private:
    ...
    void HandleWindowSizeChanged()
    {
        ...
        m_game->OnWindowSizeChanged(outputWidth, outputHeight, m_DPI, rotation);
    }
};
